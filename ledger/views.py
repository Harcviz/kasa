...
def _load_shareholders_safe() -> List[Shareholder]:
    return list(Shareholder.objects.filter(active=True).order_by("-percent", "name"))
...
def dashboard(request):
    ...
    today_qs = transactions.filter(timestamp__date=today)
    yesterday_qs = transactions.filter(timestamp__date__lte=yesterday)

    today_totals = {
        Transaction.Account.CASH: _account_totals(today_qs.filter(account=Transaction.Account.CASH)),
        Transaction.Account.BANK: _account_totals(today_qs.filter(account=Transaction.Account.BANK)),
    }
    prev_closing = {
        Transaction.Account.CASH: _account_totals(yesterday_qs.filter(account=Transaction.Account.CASH)),
        Transaction.Account.BANK: _account_totals(yesterday_qs.filter(account=Transaction.Account.BANK)),
    }

    account_cards = []
    # Sadece Nakit ve Banka tekli kartlar goster
    for code, label in (
        ...
        today_summary = today_totals[code]
        prev_summary = prev_closing[code]
        overall = {
            "incoming": prev_summary["incoming"] + today_summary["incoming"],
            "outgoing": prev_summary["outgoing"] + today_summary["outgoing"],
            "net": prev_summary["net"] + today_summary["net"],
        }
    ...
    prev_total = prev_closing[Transaction.Account.CASH]["net"] + prev_closing[Transaction.Account.BANK]["net"]
    today_total = today_totals[Transaction.Account.CASH]["net"] + today_totals[Transaction.Account.BANK]["net"]
    overall_total = prev_total + today_total
    context = {
        "now": now,
        "message": message,
        "error": error,
